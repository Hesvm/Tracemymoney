...
const DockButton = React.forwardRef<HTMLButtonElement, {
  ...
  return (
    <button
      ref={ref}
      className={`grid size-12 place-items-center rounded-full bg-white text-[#989ba8] shadow-dock transition hover:bg-[#f7f6f3] hover:text-[#626677] active:scale-95 ${className}`}
      onClick={onClick}
      aria-label={label}
      type="button"
    >
...
export function BottomNav({
  ...
  return (
    <nav
      className="fixed left-1/2 z-20 flex -translate-x-1/2 items-center gap-2 max-w-[calc(100vw-16px)]"
    ...
    >
      <DockButton label="Add item" onClick={onAddClick} ref={addButtonRef}>
        <Plus className="size-[26px]" strokeWidth={1.9} />
      </DockButton>
      <div className="flex h-12 items-center gap-1 rounded-full bg-white px-3 text-[#30333b] shadow-dock max-w-[220px] overflow-hidden">
